    ...
    public void hienThi() {
        for (int i = 0; i < nv.size(); i++) {
            if (nv.get(i).id.compareTo(nhom8_project.view.nhanvien.NhanVienForm.MANHANVIEN) == 0) {
                outPutID.setText("Mã nhân viên: " + nv.get(i).id);
                outPutUseName.setText("Tên đăng nhập: " + nv.get(i).usename);
                outPutTen.setText("Họ tên: " + nv.get(i).name);
                outPutGioiTinh.setText("Giới tính: " + nv.get(i).sex);
                outPutNgaySinh.setText("Ngày sinh: " + nv.get(i).birthday);
                outPutSDT.setText("Số điện thoại: " + nv.get(i).phone);
                outputDiaChi.setText("Địa chỉ: " + nv.get(i).address);
